...
export function moveAI(player2, env, prediction) {
	...
	if (AIAction == 0 && !env.border.up.box.intersectsBox(playerBox2)) {
		player2.paddle.mesh.translateY(0.15);
		player2.light.position.y += 0.15;
	}
	else if (AIAction == 1 && !env.border.down.box.intersectsBox(playerBox2)) {
		player2.paddle.mesh.translateY(-0.15);
		player2.light.position.y -= 0.15;
	}
}
...
export async function trainModel(model, epochs) {
	...
	const actionTensor = tf.tensor2d(actions, [numSamples, 2]);
	
	// Train the model
	await model.fit(stateTensor, actionTensor, {epochs: 1000, batchSize: 64});
...
export function storeData(env, player2, action) {
	...
	let actionArray = (action === 1) ? [1, 0] : [0, 1];
